...
class Embedding:
    ...
    def load(cls, path, normalize=True, add_context=False, **kwargs):
        if(os.path.exists(path+"-w.npy")):
            ...
            try:
                w2v=gensim.models.KeyedVectors.load_word2vec_format(path,binary=True,unicode_errors='ignore')
            except:
                w2v=gensim.models.KeyedVectors.load_word2vec_format(path,binary=False,unicode_errors='ignore')
            mat = w2v.vectors
            iw = list(w2v.vocab.keys())
        else:
